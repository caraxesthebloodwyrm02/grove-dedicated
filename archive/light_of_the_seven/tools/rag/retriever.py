...
try:
    from grid.rag.contracts import BaseRetriever, normalize_results
...
class Retriever(BaseRetriever if HAS_CONTRACTS else object):  # type: ignore[misc]
    ...
    def __init__(
        self, store: Union[VectorStore, Any], embedding_provider: Optional[EmbeddingProvider] = None
    ) -> None:
        self.store = store
        self.embedding_provider = embedding_provider
        # Detect if this is an InMemoryIndex (has .search method returning ScoredChunk)
        self._is_memory_index = hasattr(store, "search") and hasattr(store, "chunks")
    ...
    def retrieve(
        ...
        if embedding is None:
            if self.embedding_provider is None:
                # Use simple word frequency as fallback
        ...
        try:
            ...
            # ChromaDBVectorStore uses (query_embedding, n_results=..., ...) and returns dict
            res = self.store.query(embedding, n_results=k)
            if isinstance(res, dict) and "documents" in res:
                docs = res["documents"]
                metas = (
                    res.get("metadatas", [{}] * len(docs))
                    if res.get("metadatas")
                    else [{}] * len(docs)
                )
                dists = res.get("distances", [0.0] * len(docs))
                # Cosine distance: 0 = identical, 2 = opposite; convert to similarity in [0,1]
                scores = [1.0 - (d / 2.0) if isinstance(d, (int, float)) else 0.0 for d in dists]
        ...
        if self._is_memory_index:
            ...
            if HAS_CONTRACTS:
                ...
                return [
                    ...
                    for doc, meta, sc in zip(docs, metas, scores, strict=True)
                ]
    ...
    def retrieve_scored(self, query: str, top_k: int = 5) -> List[Any]:
        ...
        if self._is_memory_index:
            ...
            return list(zip(docs, scores, metas, strict=True))  # fallback
    ...
    def retrieve_with_context(
        ...
        for doc, meta, _score in zip(docs, metadatas, scores, strict=True):
            # Format document with its metadata
            if meta.get("type") == "chunk" and "path" in meta:
                formatted = (
                    f"[Source: {meta['path']} (chunk {meta.get('chunk_index', '?')})]\n{doc}"
                )
            elif "path" in meta:
    ...
    def search_by_metadata(
        self, filters: Dict[str, Any], k: Optional[int] = None
    ) -> Tuple[List[str], List[Dict[str, Any]], List[float]]:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        for doc, meta in zip(self.store.docs, self.store.metadatas, strict=True):
        ...
        return {
            ...
            "average_document_length": total_length / len(self.store.docs)
            if self.store.docs
            else 0,
