...
class GeminiSettings:
    ...
    endpoint: GeminiEndpointSettings = field(
        default_factory=GeminiEndpointSettings.from_env
    )
    model: GeminiModelSettings = field(default_factory=GeminiModelSettings.from_env)
    rate_limit: GeminiRateLimitSettings = field(
        default_factory=GeminiRateLimitSettings.from_env
    )
    ...
    def from_env(cls) -> "GeminiSettings":
        ...
        return cls(
            ...
            enable_function_calling=_parse_bool(
                env.get("GEMINI_FUNCTION_CALLING", "true")
            ),
...
_gemini_settings: Optional[GeminiSettings] = None
