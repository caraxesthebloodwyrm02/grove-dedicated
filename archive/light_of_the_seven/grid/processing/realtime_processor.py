...
class EmergencyRealtimeProcessor:
    ...
    def register_handler(
        self, flow_type: RealtimeFlow, handler: Callable[[Dict[str, Any]], Any]
    ) -> None:
    ...
    def process(
        ...
        try:
            ...
            self._stats["by_type"][flow_type.value] = (
                self._stats["by_type"].get(flow_type.value, 0) + 1
            )
            self._throttle_window.append(start_time)

            # Clean old throttle entries
            cutoff = datetime.now(timezone.utc).replace(second=0, microsecond=0) - timedelta(
                minutes=1
            )
