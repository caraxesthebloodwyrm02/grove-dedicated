...
class PromptContext(BaseModel):
    ...
    operation_type: Optional[str] = Field(default=None, description="Operation type")
...
class Prompt(BaseModel):
    ...
    prompt_id: str = Field(
        default_factory=lambda: str(uuid4()), description="Unique prompt identifier"
    )
    ...
    # Usage
    user_id: Optional[str] = Field(default=None, description="User who owns/uses this prompt")
    org_id: Optional[str] = Field(default=None, description="Organization context")
    ...
    template_defaults: Dict[str, str] = Field(
        default_factory=dict, description="Template default values"
    )
