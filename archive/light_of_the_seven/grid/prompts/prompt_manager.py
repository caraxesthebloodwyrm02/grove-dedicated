...
class PromptManager:
    ...
    def get_prompt(
        ...
        if user_id:
            prompt = self.store.get_prompt_by_name(
                name, user_id=user_id, source=PromptSource.USER_CUSTOM
            )
            if prompt:
                return prompt
        ...
        if org_id:
            prompt = self.store.get_prompt_by_name(
                name, org_id=org_id, source=PromptSource.ORG_DEFAULT
            )
            if prompt:
                return prompt
    ...
    def _invalidate_cache(self, user_id: Optional[str], org_id: Optional[str]) -> None:
