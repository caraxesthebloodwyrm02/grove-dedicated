...
class Penalty(BaseModel):
    ...
    penalty_id: str = Field(
        default_factory=lambda: str(uuid4()), description="Unique penalty identifier"
    )
    ...
    rule_violated: Optional[str] = Field(default=None, description="Rule that was violated")
    duration: Optional[timedelta] = Field(default=None, description="Penalty duration")
    expires_at: Optional[datetime] = Field(default=None, description="When penalty expires")
...
class RuleViolation(BaseModel):
    ...
    penalty_applied: Optional[str] = Field(
        default=None, description="Penalty ID if penalty was applied"
    )
    created_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))
...
class Rule(BaseModel):
    ...
    penalty_type: PenaltyType = Field(
        default=PenaltyType.WARNING, description="Default penalty type"
    )
    ...
    check_function: Optional[str] = Field(default=None, description="Function name to check rule")
    metadata: Dict[str, Any] = Field(default_factory=dict, description="Additional metadata")
    created_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))
...
class DisciplineAction(BaseModel):
    ...
    duration: Optional[timedelta] = Field(default=None, description="Action duration")
    expires_at: Optional[datetime] = Field(default=None, description="When action expires")
...
class DisciplineManager:
    ...
    def register_rule_checker(
        self, rule_id: str, checker: Callable[[Dict[str, Any]], bool]
    ) -> None:
