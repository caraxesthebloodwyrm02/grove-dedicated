...
class TraceManager:
    ...
    def trace_action(
        ...
        try:
            ...
            trace.complete(success=False, error=str(e), error_traceback=traceback.format_exc())
            raise
        finally:
    ...
    def get_active_trace(self, trace_id: Optional[str] = None) -> Optional[ActionTrace]:
    ...
    def get_trace_chain(self, trace_id: str) -> list[ActionTrace]:
        ...
        while current_id:
            trace = (
                self.store.get_trace(current_id)
                if self.store
                else self._active_traces.get(current_id)
            )
            if not trace:
                break
