    ...
    def embed(self, text: str) -> Dict[str, float]:
        ...
        try:
            result = subprocess.run(
                ["ollama", "run", self.model, text], capture_output=True, text=True, check=True
            )
...
def test_chunk_embedding_and_query(tmp_path: Path):
    ...
    # Use SimpleEmbedding so query embedding matches store (list); avoid tfidf num_docs=0 in indexer
    embed_provider = SimpleEmbedding(use_tfidf=False)
    ...
    doc_count = store.count() if hasattr(store, "count") else len(getattr(store, "docs", []))
    assert doc_count >= 1, "Expected at least one indexed document"

    retr = Retriever(store, embedding_provider=embed_provider)
...
def test_ollama_rag_demo(tmp_path: Path):
    ...
    docs = [
        "The Grid system uses RAG to enhance LLM responses with relevant context.",
        "Vector stores search through embedded documents using similarity metrics.",
...
def test_index_and_retrieve_basic():
    docs = [
        Doc(
            doc_id="d1",
            text="Apple pie is delicious and often made with cinnamon.",
...
def test_ragqa_returns_answer_and_sources():
    ...
    found = any(
        "eiffel" in (s.get("doc_id") or "") or "d1" in s.get("doc_id", "") for s in out["sources"]
    )
...
def test_ragqa_with_llm_adapter():
    ...
    docs = [
        Doc(
            ...
            metadata={"title": "mercury"},
        ),
        Doc(
            doc_id="d2",
            text="Venus is the second planet and has a thick atmosphere.",
            metadata={"title": "venus"},
        ),
    ]
    idx = InMemoryIndex()
