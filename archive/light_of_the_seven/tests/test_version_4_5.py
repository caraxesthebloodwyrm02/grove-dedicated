...
class TestIntelligenceV45:
    ...
    async def test_basic_processing(self, intelligence):
        ...
        result = await intelligence.process_sensory_input(
            source="test", data={"value": 42}, modality="structured"
        )
    ...
    async def test_pattern_prediction(self, intelligence):
        ...
        for i in range(5):
            await intelligence.process_sensory_input(
                source=f"source_{i}", data={"value": i}, modality="structured"
            )

        # Next call should have predictions
        result = await intelligence.process_sensory_input(
            source="final", data={"value": 100}, modality="structured"
        )
    ...
    async def test_cross_layer_entanglement(self, intelligence):
        ...
        for i, mod in enumerate(modalities):
            await intelligence.process_sensory_input(
                source=f"source_{i}", data={"confidence": 0.9}, modality=mod
            )
