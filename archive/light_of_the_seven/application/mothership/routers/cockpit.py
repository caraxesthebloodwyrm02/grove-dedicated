...
from ..models.cockpit import OperationMode, SystemStatus
from ..schemas import (
    ApiResponse,
...
async def health_check(
    ...
    checks = {
        ...
        "components": all(c.is_healthy() for c in state.components.values())
        if state.components
        else True,
        "alerts": sum(1 for a in state.alerts.values() if not a.is_resolved)
        < 100,  # Alert threshold
    }
...
async def get_state(
    ...
    summary = CockpitSummarySchema(
        ...
        pending_tasks=sum(
            ...
            if op.status.value in {"pending", "queued"}
        ),
...
async def change_mode(
    ...
    # Log mode change
    logger.info(
        f"Mode change: {previous_mode} -> {new_mode} "
        f"by user {context.get('user_id')} "
...
async def run_diagnostics(
    ...
    try:
        ...
        checks.append(
            DiagnosticResult(
                ...
                message="State store accessible"
                if store_ok
                else "State store unavailable",
    ...
    checks.append(
        DiagnosticResult(
            ...
            duration_ms=(time.time() - check_start) * 1000,
            details={"total": session_count, "active": active_sessions}
            if include_details
            else {},
        )
    )
    ...
    checks.append(
        DiagnosticResult(
            ...
            duration_ms=(time.time() - check_start) * 1000,
            details={"running": running_ops, "total": total_ops}
            if include_details
            else {},
        )
    )
    ...
    try:
        ...
        checks.append(
            DiagnosticResult(
                ...
                duration_ms=(time.time() - check_start) * 1000,
                details={"estimated_state_bytes": state_size}
                if include_details
                else {},
            )
        )
...
async def initiate_shutdown(
    ...
    cockpit.alerts.create_alert(
        ...
        severity=cockpit.alerts._state.alerts.get(
            "severity", cockpit.alerts._state
        ).__class__.__bases__[0]
        if hasattr(cockpit.alerts, "_state")
        else "warning",
        source="cockpit_api",
    )
...
async def get_statistics(
    ...
    stats = {
        ...
        "operations": {
            ...
            "success_rate": (
                len(completed_ops) / len(operations) * 100 if operations else 100
            ),
        ...
        "components": {
            ...
            "unhealthy": sum(
                1 for c in state.components.values() if not c.is_healthy()
            ),
        },
        "alerts": {
            ...
            "by_severity": {
                severity: sum(
                    1 for a in state.alerts.values() if a.severity.value == severity
                )
