...
class SessionRepository(BaseRepository[CockpitSession]):
    ...
    async def get(self, id: str) -> Optional[CockpitSession]:
        """Get session by ID."""
...
class ComponentRepository(BaseRepository[CockpitComponent]):
    ...
    async def get(self, id: str) -> Optional[CockpitComponent]:
        """Get component by ID."""
    ...
    async def get_by_name(self, name: str) -> Optional[CockpitComponent]:
    ...
    async def get_by_type(self, component_type: str) -> List[CockpitComponent]:
        ...
        from ..models.cockpit import ComponentType

        return [
            c
            for c in self._store.components.values()
            if c.component_type.value == component_type
        ]
    ...
    async def get_unhealthy(self) -> List[CockpitComponent]:
        ...
        return [
            c
            for c in self._store.components.values()
            if c.health == ComponentHealth.UNHEALTHY
        ]
    ...
    async def update_health(
        ...
        message: str = "",
    ) -> Optional[CockpitComponent]:
        """Update component health status."""
        from ..models.cockpit import ComponentHealth as CH
...
class AlertRepository(BaseRepository[CockpitAlert]):
    ...
    async def get(self, id: str) -> Optional[CockpitAlert]:
        """Get alert by ID."""
    ...
    async def get_by_severity(self, severity: "AlertSeverity") -> List[CockpitAlert]:
        """Get alerts by severity."""
        from ..models.cockpit import AlertSeverity as AS
    ...
    async def get_unacknowledged(self) -> List[CockpitAlert]:
        ...
        return [
            a for a in self._store.alerts.values() if not a.acknowledged and a.is_active
        ]

    async def get_by_component(self, component_id: str) -> List[CockpitAlert]:
        """Get alerts for a specific component."""
        return [
            a for a in self._store.alerts.values() if a.component_id == component_id
        ]

    async def acknowledge(self, alert_id: str, user_id: str) -> Optional[CockpitAlert]:
    ...
    async def resolve(self, alert_id: str) -> Optional[CockpitAlert]:
        """Resolve an alert."""
        alert = await self.get(alert_id)
...
def get_unit_of_work(store: Optional[StateStore] = None) -> UnitOfWork:
